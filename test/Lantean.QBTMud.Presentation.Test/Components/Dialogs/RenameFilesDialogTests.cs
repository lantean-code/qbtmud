...
namespace Lantean.QBTMud.Presentation.Test.Components.Dialogs
{
    ...
    {
        private const string _preferencesKey = "RenameFilesDialog.MultiRenamePreferences";
        ...
        public async Task GIVEN_RememberedPreferencesAndHierarchy_WHEN_Rendered_THEN_PopulatesStateAndFlattensTree()
        {
            ...
            apiClientMock.Setup(c => c.GetTorrentContentsAsync("Hash", It.IsAny<IEnumerable<int>?>(), It.IsAny<CancellationToken>())).ReturnsSuccessAsync(Array.Empty<FileData>());
        ...
        public async Task GIVEN_RememberPreferencesEnabled_WHEN_OptionsChanged_THEN_PreferencesPersisted()
        {
            ...
            var json = await TestContext.LocalStorage.GetItemAsStringAsync(_preferencesKey, Xunit.TestContext.Current.CancellationToken);
        ...
        {
            await TestContext.LocalStorage.SetItemAsStringAsync(_preferencesKey, "{\"rememberPreferences\":true}", Xunit.TestContext.Current.CancellationToken);
            TestContext.AddSingletonMock<IApiClient>(MockBehavior.Strict);
        ...
        public async Task GIVEN_ReplaceAllSelected_WHEN_Submitted_THEN_RenamesFilesAndFolders()
        {
            ...
            apiClientMock.Setup(c => c.RenameFileAsync("Hash", filePaths.OldPath, filePaths.NewPath)).ReturnsSuccess(Task.CompletedTask);
            apiClientMock.Setup(c => c.RenameFolderAsync("Hash", folderPaths.OldPath, folderPaths.NewPath)).ReturnsSuccess(Task.CompletedTask);
