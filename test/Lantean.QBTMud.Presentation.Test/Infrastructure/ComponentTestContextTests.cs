...
namespace Lantean.QBTMud.Presentation.Test.Infrastructure
{
    public class ComponentTestContextTests : IDisposable
    {
        ...
        {
            await _target.LocalStorage.SetItemAsync("Number", 42, TestContext.Current.CancellationToken);

            var value = await _target.LocalStorage.GetItemAsync<int>("Number", TestContext.Current.CancellationToken);
            value.Should().Be(42);
        }
